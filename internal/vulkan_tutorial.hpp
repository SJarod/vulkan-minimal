...
inline void vulkan_load_symbols()
{
...
namespace Instance
{
	...
	inline VkInstance vulkan_create_instance()
	{
	...
	inline void vulkan_destroy_instance(VkInstance& instance)
	{
		vkDestroyInstance(instance, nullptr);
	}
	inline void vulkan_enumerate_available_instance_layers()
	{
	...
	inline void vulkan_enumerate_available_instance_extensions()
	{
	...
	namespace Debug
	{
		inline VkDebugUtilsMessengerEXT vulkan_create_debug_messenger(VkInstance instance)
		{
		...
		inline void vulkan_destroy_debug_messenger(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger)
		{
...
namespace Device
{
	...
	namespace Client
	{
		inline bool is_device_suitable(VkPhysicalDevice physicalDevice)
		{
		...
		inline VkPhysicalDevice vulkan_get_suitable_physical_device(VkInstance instance)
		{
			auto physicalDevices = vulkan_get_physical_devices(instance);
	...
	inline bool is_device_compatible_with_extensions(VkPhysicalDevice physicalDevice, const std::vector<const char*> deviceExtensions)
	{
		uint32_t extensionCount = 0;
	...
	namespace Memory
	{
		inline uint32_t find_memory_type_index(VkPhysicalDevice physicalDevice, VkMemoryRequirements requirements, VkMemoryPropertyFlags properties)
		{
	...
	inline VkDevice vulkan_create_logical_device(VkInstance instance, VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
	{
		auto graphicsFamily = Queue::vulkan_find_queue_family_operation(physicalDevice, VK_QUEUE_GRAPHICS_BIT);
		auto presentFamily = Queue::vulkan_find_queue_family_presentation(physicalDevice, surface);
	...
	inline void vulkan_destroy_logical_device(VkDevice device)
	{
	...
	namespace Queue
	{
		inline std::optional<uint32_t> vulkan_find_queue_family_operation(VkPhysicalDevice physicalDevice, VkQueueFlags flags)
		{
		...
		inline std::optional<uint32_t> vulkan_find_queue_family_presentation(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
		{
			std::optional<uint32_t> index;
		 ...
		 */
		inline VkQueue vulkan_get_device_queue(VkDevice device, uint32_t queueFamilyIndex)
		{
			VkQueue queue;
...
namespace Presentation
{
	namespace Surface
	{
		inline VkSurfaceKHR vulkan_create_surface(GLFWwindow* window, VkInstance* instance)
		{
			VkSurfaceKHR surface;
			wsi_create_presentation_surface(instance, window, &surface);
			return surface;
		}
		inline void vulkan_destroy_surface(VkInstance instance, VkSurfaceKHR surface)
		{
		...
		inline std::vector<VkSurfaceFormatKHR> get_surface_available_formats(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
		{
		...
		inline std::vector<VkPresentModeKHR> get_surface_available_present_modes(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
		{
		...
		namespace Client
		{
			...
			inline VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availableModes)
			{
	...
	namespace SwapChain
	{
		inline VkSwapchainKHR vulkan_create_swap_chain(VkPhysicalDevice physicalDevice, VkDevice device, VkSurfaceKHR surface)
		{
			VkSwapchainSupportDetails support = querySwapchainSupport(physicalDevice);
		...
		inline void vulkan_destroy_swap_chain(VkDevice device, VkSwapchainKHR swapchain)
		{
			vkDestroySwapchainKHR(device, swapchain, nullptr);
		}

		inline std::vector<VkImage> vulkan_get_swap_chain_images(VkDevice device, VkSwapchainKHR swapchain)
		{
		...
		inline std::vector<VkImageView> vulkan_create_swap_chain_image_views(VkDevice device, VkSwapchainKHR swapchain, std::vector<VkImage> images)
		{
		...
		inline void vulkan_destroy_swap_chain_image_views(VkDevice device, std::vector<VkImageView> swapchainImageViews)
		{
...
namespace Pipeline
{
	namespace Shader
	{
		inline VkShaderModule vulkan_create_shader_module(VkDevice device, const std::vector<char>& code)
		{
		...
		inline void vulkan_destroy_shader_module(VkDevice device, VkShaderModule module)
		{
			vkDestroyShaderModule(device, module, nullptr);
		}

		inline VkPipelineLayout vulkan_create_pipeline_layout(VkDevice device)
		{
		...
		inline void vulkan_destroy_pipeline_layout(VkDevice device, VkPipelineLayout pipelineLayout)
		{
	...
	{
		inline VkRenderPass vulkan_create_render_pass(VkDevice device)
		{
		...
		inline void vulkan_destroy_render_pass(VkDevice device, VkRenderPass renderPass)
		{
			vkDestroyRenderPass(device, renderPass, nullptr);

		}

		inline std::vector<VkFramebuffer> vulkan_create_framebuffers(VkDevice device, std::vector<VkImageView> swapchainImageViews)
		{
		...
		inline void vulkan_destroy_framebuffers(VkDevice device, std::vector<VkFramebuffer> framebuffers)
		{
	...
	inline VkPipeline vulkan_create_pipeline(VkDevice device, VkRenderPass renderPass)
	{
		...
		VkShaderModule vsModule = Shader::vulkan_create_shader_module(device, vs);
		VkShaderModule fsModule = Shader::vulkan_create_shader_module(device, fs);
		...
		// shader variables
		VkPipelineLayout pipelineLayout = Shader::vulkan_create_pipeline_layout(device);
		...
		Shader::vulkan_destroy_shader_module(device, vsModule);
		Shader::vulkan_destroy_shader_module(device, fsModule);

		return pipeline;
	}
	inline void vulkan_destroy_pipeline(VkDevice device, VkPipeline pipeline)
	{
...
namespace Command
{
	inline VkCommandPool vulkan_create_command_pool(VkPhysicalDevice physicalDevice, VkDevice device)
	{
		VkCommandPoolCreateInfo createInfo = {
			.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO,
			.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,
			.queueFamilyIndex = Device::Queue::vulkan_find_queue_family_operation(physicalDevice, VK_QUEUE_GRAPHICS_BIT).value(),
		};
	...
	inline void vulkan_destroy_command_pool(VkDevice device, VkCommandPool commandPool)
	{
		vkDestroyCommandPool(device, commandPool, nullptr);
	}

	inline VkCommandBuffer vulkan_allocate_command_buffer(VkDevice device, VkCommandPool commandPool)
	{
	...
	namespace Client
	{
		inline void vulkan_record_command_buffer(VkCommandBuffer cb, uint32_t imageIndex)
		{
...
namespace Parallel
{
	inline VkSemaphore vulkan_create_semaphore(VkDevice device)
	{
	...
	inline void vulkan_destroy_semaphore(VkDevice device, VkSemaphore semaphore)
	{
		vkDestroySemaphore(device, semaphore, nullptr);

	}

	inline VkFence vulkan_create_fence(VkDevice device)
	{
	...
	inline void vulkan_destroy_fence(VkDevice device, VkFence fence)
	{
...
namespace Memory
{
	inline VkBuffer vulkan_create_buffer(VkDevice device, uint64_t size)
	{
	...
	inline void vulkan_destroy_buffer(VkDevice device, VkBuffer buffer)
	{
		vkDestroyBuffer(device, buffer, nullptr);
	}

	inline VkDeviceMemory vulkan_allocate_memory(VkPhysicalDevice physicalDevice, VkDevice device, VkBuffer buffer)
	{
		...
		VkMemoryAllocateInfo allocInfo = {
			...
			.memoryTypeIndex = Device::Memory::find_memory_type_index(physicalDevice, memReq, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
		};
	...
	inline void vulkan_free_memory(VkDevice device, VkDeviceMemory memory)
	{
		vkFreeMemory(device, memory, nullptr);
	}
	inline void vulkan_bind_memory_to_buffer(VkDevice device, VkBuffer buffer, VkDeviceMemory memory)
	{
		vkBindBufferMemory(device, buffer, memory, 0);
	}

	inline void vulkan_copy_data_to_memory(VkDevice device, VkDeviceMemory memory, uint64_t size)
	{
